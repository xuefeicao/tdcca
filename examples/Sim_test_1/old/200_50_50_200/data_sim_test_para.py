...
n, d_1, d_2, T = 200, 50, 50, 200
ext = str(n) + '_' + str(d_1) + '_' + str(d_2) + '_' + str(T) + '/'
folder_name = '/gpfs_home/xcao1/scratch/research_Rossi_1/data/tcca/test_with_val/test_para_1/' + ext 
...
for i in range(num_sim):
    ...
    for t in range(T/2):
        ...
        X_ans = scale(X[:,:,t], with_std=False)
        Y_ans = scale(Y[:,:,t], with_std=False)
        
        X_ans = X_ans/(np.sum(X_ans**2,axis=0)**0.5)
        Y_ans = Y_ans/(np.sum(Y_ans**2,axis=0)**0.5)
...
lam = [10**i for i in range(3,5)] + [2*(10**i) for i in range(3,5)]
mu = [10**i for i in range(3,5)] + [2*(10**i) for i in range(3,5)]


# scaling, by_pass, pre_sign, with_sign, with_init 
multi_sim(data, lam, mu, nu, folder_name_all, real_W, num_cores=8, admm_method='admm_2',max_iter=5000, tol_admm=1e-2, T_dif = [T/2-1], folds=5, num_val=2, scaling=False, pre_sign=True, with_sign=True, by_pass=False, calculate_init=True)
