...
def admm_comp(data_X, val_config, method, k_fold, with_init={}, test=False):
    ...
    def obj_value_2(W_i, W_i_t, W_i_h, Phi, Theta, i, ll, SVD_ijt, out_put=False):
        """
        compute the value of objective funciton (ADMM)
        """
        loss = 0
    ...
    def admm_sep_2(i_ll,P, SVD_ijt, USP, tol=1e-2, max_iter=1000, with_init=with_init, lam=lam, mu=mu, nu=nu, A_inv=A_inv, cvx_val = None):
        ...
        loss_0 = obj_value(W_i, W_i_t, W_i_h, Phi, Theta, i, ll, SVD_ijt)
        loss_1 = loss_0 + 1
        l5 = 1e32
        _conv = True
        iter = 0
        ...
        while iter <= max_iter:
            ...
            if iter%10 == 0:
                ...
                if (np.sum(np.absolute(W_i-W_i_h)+np.absolute(W_i-W_i_t))/(np.sum(abs(W_i)) + 1e-2) < tol) and abs(loss_0 -loss_1)/loss_0 < tol:
        ...
        T_dif_i = obj_value(W_i, W_i_t, W_i_h, Phi, Theta, i, ll, SVD_ijt, out_put=True)
    ...
    for i in range(N):
        for ll in range(l):
            if test:
                print 'haha1'
                _, cvx_val = cvx_1((i,ll), SVD_ijt=SVD_ijt, tol=tol_admm, max_iter=max_iter, lam=lam*1.0, mu=mu*1.0, nu=nu*1.0)
                print 'haha2'
            ...
            W[(i,ll)], _conv, T_dif_i, W_t[(i,ll)], W_h[(i,ll)], Theta_all[(i,ll)], Phi_all[(i,ll)] = admm_sep((i,ll), P=P, SVD_ijt=SVD_ijt, USP=USP, tol=tol_admm, max_iter=max_iter, with_init=with_init, lam = 1.0*lam, mu = 1.0*mu, nu = 1.0*nu, cvx_val=cvx_val)
            if test:
